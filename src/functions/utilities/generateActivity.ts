...
const defaultActivity: Record<string, string> = {
...
const generateActivity = async (
  client: Client,
  channel: Channel,
  { custom: id, name }: { custom?: string; name?: string }
): Promise<object> => {
  if (!client.isReady() || !(client instanceof Client)) {
    throw new Error(
